...
def contains_executables(directory):
    ...
    return any(
        os.access(os.path.join(directory, f), os.X_OK) for f in os.listdir(directory)
    )
...
def add_subdirectories_to_path(base_dir):
    ...
    subdirectories = [
        ...
        and contains_executables(os.path.join(base_dir, d))
    ]
...
def auto_discover_paths():
    ...
    discovered_paths = [
        ...
        if os.path.isdir(path) and contains_executables(path)
    ]
...
def process_user_input():
    ...
    add_paths = [
        path.strip() for path in user_input if path and not path.startswith("-")
    ]
...
def clean_and_manage_path(base_paths=[], add_paths=[], remove_paths=[]):
    ...
    all_paths = set(
        os.environ.get("PATH", "").split(os.pathsep) + base_paths + add_paths
    )
