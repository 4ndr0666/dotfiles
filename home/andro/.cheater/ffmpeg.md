...
In this example, `input.mov` is converted to `output.webm` with a constant rate factor of `10` (lower is higher quality) at a bitrate of `1M`.
Changing the bitrate to something lower (e.g. `700K`) will result in lower file sizes and lower quality.
If your video does not have audio, you may leave off the `-acodec libvorbis` part.
...
**VP9**
VP9 can encode videos at half the file size. You can check out Google's [VP9 encoding guide](https://sites.google.com/a/webmproject.org/wiki/ffmpeg/vp9-encoding-guide) for their recommend settings or the [FFmpeg VP9 guide](https://trac.ffmpeg.org/wiki/Encode/VP9).
...
This requires the `libvidstab` filter during mpv compilation.
...
Reduce a 100mb video to 9mb with very little change in video quality:
...
## Create tiled mosaic by scene change in a video file
