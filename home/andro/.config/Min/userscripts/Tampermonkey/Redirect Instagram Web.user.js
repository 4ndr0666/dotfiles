...
if (location.pathname === '/robots.txt' && location.search.startsWith('?z5m4F=')) {
  ...
  */
  ...
  F_AUTO_REDIRECT && location.replace('https://wizstat.com' + location.pathname)
...
display: none !important;
      ...
      quicklink.listen({
        ...
        ignores: [
          (uri, elem) => {
            ...
            let isP = /^https\:\/\/wizstat\.com\/p\/[^\/]+\/$/.test(uri);
            if (!isP) return true;
            return false;
  ...
  document.addEventListener('load', (evt) => {
    ...
    if (src.length > 30 && src.indexOf('.wizstat.com/') > 0 && src.indexOf('.cdninstagram.com/') > 0) {
      ...
      if (p) {
        ...
        })).filter(o => (o.href.indexOf('wizstat.com/') < 0));
        let newUrl = '';
