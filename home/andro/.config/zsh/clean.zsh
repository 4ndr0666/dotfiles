...
rmdupes_suffix() {
	...
	if [[ "$1" == "-h" || "$1" == "--help" ]]; then
		...
		echo "  Deletes files ending in ' N.(mp4|mov|m4v|3gp)' where N is a number."
		return 0
	fi

	# Parse flag and optional directory
	for arg in "$@"; do
		case "$arg" in
		-f)
			force=1
	...
	# Match any case variant: .mp4, .mov, .m4v, .3gp (case-insensitive)
	find "$dir" -type f -iregex '.* [0-9]+\.\(mpg\|gif\|webm\|avi\|wmv\|flv\|mp4\|mov\|m4v\|3gp\)$' | while IFS= read -r file; do
		if ((force)); then
			echo "[DELETING] $file"
			rm -- "$file"
		else
