...
prompt_cookie_update() {
 ...
 if [[ -n "$BASH_VERSION" ]]; then
     # Bash syntax for associative arrays (0-based indexing)
     domains=( "${!YTDLP_COOKIES_MAP[@]}" )
 elif [[ -n "$ZSH_VERSION" ]]; then
     # Zsh syntax for associative arrays (1-based indexing)
...
EOF
 ...
 # Check for '--' separator to capture extra yt-dlp args
 if [[ "$1" == "--" ]]; then
     shift
     ytdlp_extra_args=( "$@" )
 fi
 ...
 # Process each URL
 for url in "$@"; do
     # If '--' is encountered, stop processing URLs
     if [[ "$url" == "--" ]]; then
         shift
         break
     fi
...
#
#   ytdlc [options] <URL> [<URL> ...] [-- yt-dlp options]
#     => Advanced domain-based cookie approach with additional yt-dlp flags
#
