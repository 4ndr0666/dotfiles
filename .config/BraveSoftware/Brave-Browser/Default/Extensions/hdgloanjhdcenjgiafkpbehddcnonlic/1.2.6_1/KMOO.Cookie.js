...
KMOO.Cookie.prototype.isValid = function () {
  ...
  if (this.validity_ === null) {
    ...
    KMOO.PolicyRegistry.getDomainPolicy(this.cookie_.domain).filter(function (policy) {
      return self.cookie_.name === policy.name
    }).forEach(function (policy) {
      self.validity_ = self.cookie_.value === policy.value
    })
  }
...
KMOO.Cookie.prototype.set = function () {
  ...
  KMOO.PolicyRegistry.getDomainPolicy(this.cookie_.domain).forEach(function (policy) {
    if (instance.isLocked()) {
      console.log('`%s` for `%s` is being set, exiting.', policy.name, policy.domain)
      return
    }
    ...
    chrome.cookies.get({ 'url': policy.url, 'name': policy.name }, function (currentCookie) {
      if (currentCookie && currentCookie.value === policy.value) return

      // Setting can also trigger the handler, and a fun infinite loop happens.
      // So we lock the cookie to avoid it being set... again.
      instance.lock()

      chrome.cookies.set(policy, function (cookie) {
        // Supply logging for this. The old code didn't so we were left
        // scratching our heads when cookies weren't set.

        if (cookie) {
          console.log('Setting opt-out cookie succeeded:', policy)
        } else {
          console.warn('Setting opt-out cookie failed:', policy)
        }
      })
    })
  })
