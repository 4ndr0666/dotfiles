...
KMOO.PolicyRegistry = (function () {
    ...
    req.onreadystatechange = function (e) {
      if (req.readyState === 4 && (req.status === 200 || req.status === 0)) {
        ...
        for (var i = temp.registry.length - 1; i >= 0; i--) {
          ...
          // Dots on host name can cause Firefox to fail (i.e. http://.example.com)
          // Chrome just seems to swallow the error and just work.
          el['url'] = 'http://' + el.host.replace(/^\./, '') + el.path
          el['domain'] = el.host
          // Domains start with `.`
          if (el['domain'][0] !== '.') {
            el['domain'] = '.' + el['domain']
  ...
  return {
    ...
    'forEach': function (callback) {
      for (var domain in registry_.byDomain) {
